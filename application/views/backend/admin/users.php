                ...
                <table id="basic-datatable" class="table table-striped table-centered mb-0">
                  <thead>
                    <tr>
                      ...
                      <th><?php echo get_phrase('login_status'); ?></th>
                      ...
                      <?php
                       foreach ($users->result_array() as $key => $user): ?>
                          <tr>
                              ...
                              <td style="outline: inherit!important; outline-offset: inherit!important; background-color: inherit!important;">
                                  
                                     
                                 <?php if($user['log_active']==1) 
                                 {
                                 ?>
                                    <button type="button" class="btn btn-info btn-icon btn-rounded btn-sm" onclick="confirm_modal('<?php echo site_url('admin/users_login_inactive/'.$user['id']); ?>');"> <i class="dripicons-lock"></i> </button>
                                 
                                 <?php
                                 }else{
                                 ?> 
                                    
                                     <button type="button" class="btn btn-light btn-icon btn-rounded btn-sm" onclick="confirm_modal('<?php echo site_url('admin/users_login_active/'.$user['id']); ?>');"> <i class="dripicons-lock-open"></i> </button>
                                 <?php
                                 }
                                 ?>
