...
class Login extends CI_Controller {
    ...
    public function validate_login($from = "") {
        ...
        if ($query->num_rows() > 0) {
            ...
            //Checking the method 
            $q = $this->db->get_where('settings', array('key' => 'login_method'));
            $ro = $q->row();
            
            $loginMehodCheck = $ro->value;
            
            if($loginMehodCheck == "1"){
                ...
                if($checkCodition){
                    ...
                    $checkLogin = array('email' => $email ,'password' => sha1($password),'log_active' => 0);
                    $checkQuery = $this->db->get_where('users', $checkLogin);
                    ...
                    if ($checkQuery->num_rows() > 0) {
                        ...
                        // User can log one time then check the status log_active
                        $this->db->where('id' , $row->id);
                        $this->db->update('users' , array('log_active' => 1));
                        ...
                        $this->session->set_flashdata('error_message',get_phrase('you_are_already_logged_in_on_another_device'));
                        redirect(site_url('home/login'), 'refresh');
                    }
                }else{
